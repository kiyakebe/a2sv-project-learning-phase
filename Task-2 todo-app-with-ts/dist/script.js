...
const deleteTask = (task_id_to_delete) => {
    const task_to_delete = document.getElementById(task_id_to_delete.toString());
    task_to_delete.remove();
    tasks = tasks.filter((task) => task[0] != task_id_to_delete);
    toggleTodoStatus();
};
const editTask = (task_id_to_edit) => {
    const task_to_edit = document.getElementById(task_id_to_edit);
    let editable_field = task_to_edit.children[0];
    editable_field.contentEditable = "true";
    editable_field.focus();
    toggleButton(task_id_to_edit, true);
};
const saveUpdate = (task_id_to_update) => {
    const task_to_edit = document.getElementById(task_id_to_update.toString());
    const editable_field = task_to_edit.children[0];
    ...
    if (!new_data) {
        ...
        tasks = tasks.map((task) => task[0] == task_id_to_update ? [task[0], new_data] : task);
    }
    toggleButton(task_id_to_update.toString(), false);
    renderTodos(tasks);
};
const cancelUpdate = (task_id_to_cancel_update) => {
    const task_to_edit = document.getElementById(task_id_to_cancel_update.toString());
    const editable_field = task_to_edit.children[0];
    editable_field.contentEditable = "false";
...
const renderNewTask = (curr_task, curr_task_id) => {
