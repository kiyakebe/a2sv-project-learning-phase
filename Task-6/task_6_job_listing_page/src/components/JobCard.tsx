...
const JobCard = ({ title, logo, description, company, location }: JobType) => {
  return (
    <div className="flex border border-gray-400 p-10 w-3/4 rounded-3xl space-x-5 mx-auto my-8 bg-white">
      <div className="block shrink-0">
        <Image src={`${logo}`} alt="Picture of the author" width={60} height={60} />
      </div>
      <div>
        <h2 className="font-semibold">{title}</h2>
        <p className="text-sm text-slate-500 my-1">
          {company} <span className="text-3xl">.</span> {location}
        </p>
